...
def create_document(
    ...
    body: DocumentCreate = None, # Hacerlo opcional evita el 400 prematuro de FastAPI
):
    """Crea un documento: draft privado a nombre tuyo, en TU empresa."""
    if body is None:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Body requerido")
...
def get_document(
    ...
    """Ve un documento por id. Mitigación de IDOR y Tenancy."""
    doc = storage.get_document(doc_id)
    if doc is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Documento no encontrado")
    
    if doc.tenant_id != current_user.tenant_id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="No podés acceder a documentos de otra empresa")
        
    if doc.visibility == "public":
        return doc
        
    if doc.owner_id != current_user.id and current_user.role != Role.ADMIN:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="No tenés permiso para ver este documento privado")
...
def update_document(
    doc_id: int,
    current_user: User = Depends(require_scope("write")),
    body: DocumentUpdate = None, # Evita rechazos de formato antes de la autorización
):
    ...
    if doc.owner_id != current_user.id and current_user.role != Role.ADMIN:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="No tenés permiso para editar este documento")
...
def delete_document(
    ...
    """Borra un documento: SOLO admin de la misma empresa."""
    doc = storage.get_document(doc_id)
    if doc is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Documento no encontrado")
...
def publish_document(
    ...
    if doc.owner_id != current_user.id and current_user.role != Role.ADMIN:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="No tenés permiso para publicar este documento")
